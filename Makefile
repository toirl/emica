...
coverage: ## Run tests quickly with the default Python.
	pytest --cov-report xml --cov-report term-missing --cov=emica tests --cov-fail-under=100
...
docker-build: dist ## Build container for this application.
	docker build -t toirl/emica -f docker/Dockerfile .
docker-build-develop: ## Build container for development in PyCharm
	docker build --target develop -t toirl/emica-develop -f docker/Dockerfile .

docker-publish: docker-build ## Publish docker container on docker.io.
	# Stable "latest" tag
	docker tag toirl/emica toirl/emica:latest
	docker push toirl/emica:latest
	# Stable major tag
	docker tag toirl/emica toirl/emica:$$(python docker/buildtags.py major)
	docker push toirl/emica:$$(python docker/buildtags.py major)
	# Stable minor tag
	docker tag toirl/emica toirl/emica:$$(python docker/buildtags.py minor)
	docker push toirl/emica:$$(python docker/buildtags.py minor)
	# Unique rev tag
	docker tag toirl/emica toirl/emica:$$(python docker/buildtags.py commit)
	docker push toirl/emica:$$(python docker/buildtags.py commit)

docker-run-develop: ## Run bash in container for development.
	docker run --rm -it -v $$(pwd):/app --entrypoint /bin/bash toirl/emica-develop

docker-run: ## Run docker container.
	docker run --rm -it toirl/emica
